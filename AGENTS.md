...
Root `version.json` is the **app-level** version surfaced in the UI/update flow — distinct from the two package versions.
...
Local defaults: seeded admin login is `admin` / `admin123`. Health check is **`GET /api/health`**.
...
**Use these — do not re-invent the same logic in a route handler.**
...
- **`backend/src/utils/orderDate.ts`** — Fake-UTC date helpers.
  ...
  - `safeDateFromBody(s)` — tolerant date parser for non-business timestamps; returns `null` on invalid.
  - `isSameCateringDay(a, b)` / `addDaysToKey(key, n)` — calendar math without DST/tz slips.
- **`backend/src/middleware/blockVendor.middleware.ts`** — `blockVendorMiddleware` rejects VENDOR role with 403. Use this on every order/checkin mutation route.
...
- **`backend/src/utils/env.ts` `ENABLE_TOKEN_ROTATION`** — feature flag for refresh-token rotation. Also flip via `Settings.enableTokenRotation` in the admin UI.
...
- **`frontend/src/utils/timezone.ts:formatToWIB`** — TZ-neutral substring slicer. Safe for all timezones.
...
- `src/services/` holds cross-cutting logic: `time.service`, `redis.service`, `cache.service` (Redis-backed), `audit.service` (`logOrder`, `getRequestContext`), `scheduler` (node-cron no-show processing), `notification.service` (hybrid Web Push + FCM), `rate-limiter.service`.
...
- **VENDOR is restricted**: explicitly blocked from creating orders and confined to vendor pages. When adding order-mutating endpoints, use `blockVendorMiddleware`.
